...
const DEFAULT_LISTINGS = [
  {
    ...
    images: [{ url: 'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=600&auto=format&fit=crop&q=80' }],
  ...
  },
  {
    ...
    images: [{ url: 'https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=600&auto=format&fit=crop&q=80' }],
  ...
  },
  {
    ...
    images: [{ url: 'https://images.unsplash.com/photo-1591337676887-a217a6970a8a?w=600&auto=format&fit=crop&q=80' }],
  ...
  },
  {
    ...
    images: [{ url: 'https://images.unsplash.com/photo-1606813907291-d86efa9b94db?w=600&auto=format&fit=crop&q=80' }],
  ...
  },
  {
    ...
    images: [{ url: 'https://images.unsplash.com/photo-1502920917128-1aa500764cbd?w=600&auto=format&fit=crop&q=80' }],
...
export default function AdminModerationPage() {
  ...
  const [takedownReason, setTakedownReason] = useState('Barang Tiruan / Indikasi Pelanggaran Aturan');
  ...
  const [localOverrides, setLocalOverrides] = useState<Record<string, { status: 'ACTIVE' | 'ARCHIVED' | 'SOLD' }>>({});

  const rawListings = (listingsData?.data && listingsData.data.length > 0) ? listingsData.data : DEFAULT_LISTINGS;
  ...
  const handleUpdateStatus = async (listingId: string, status: 'ACTIVE' | 'ARCHIVED', listingTitle?: string) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 font-medium mt-1">
            Audit kepatuhan konten iklan barang bekas, cegah peredaran barang tiruan/terlarang, dan kelola takedown iklan.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xs hover:shadow-md transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black text-slate-400 uppercase tracking-wider">Total Katalog Iklan</span>
            <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-slate-100 text-slate-700">
              <Package className="h-4.5 w-4.5" />
            </div>
          </div>
          <div className="mt-3">
            <h3 className="text-2xl font-black text-slate-900 tracking-tight font-mono">{listings.length} Barang</h3>
            <span className="text-[11px] text-slate-500 font-medium block mt-1">Katalog Aktif & Terarsip</span>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-emerald-200 bg-emerald-50/20 p-5 shadow-xs hover:shadow-md transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black text-emerald-800 uppercase tracking-wider">Tayang Aktif</span>
          ...
          <div className="mt-3">
            <h3 className="text-2xl font-black text-emerald-950 tracking-tight font-mono">{activeCount} Iklan</h3>
            <span className="text-[11px] text-emerald-700 font-bold block mt-1">Siap Dibeli Pembeli</span>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-rose-200 bg-rose-50/20 p-5 shadow-xs hover:shadow-md transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black text-rose-800 uppercase tracking-wider">Ditakedown / Arsip</span>
          ...
          <div className="mt-3">
            <h3 className="text-2xl font-black text-rose-950 tracking-tight font-mono">{archivedCount} Iklan</h3>
            <span className="text-[11px] text-rose-700 font-bold block mt-1">Dinonaktifkan Admin</span>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xs hover:shadow-md transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black text-slate-400 uppercase tracking-wider">Barang Terjual</span>
            <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 border border-blue-100">
              <TrendingUp className="h-4.5 w-4.5" />
            </div>
          </div>
          <div className="mt-3">
            <h3 className="text-2xl font-black text-slate-900 tracking-tight font-mono">{soldCount} Terjual</h3>
            <span className="text-[11px] text-blue-700 font-bold block mt-1">Transaksi Selesai</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-slate-100 border border-slate-200 w-fit overflow-x-auto">
          ...
          >
            ...
            <span
              className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
                activeTab === 'ACTIVE' ? 'bg-emerald-100 text-emerald-900' : 'bg-slate-200 text-slate-700'
              }`}
            >
              {activeCount}
            </span>
          ...
          >
            ...
            <span
              className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
                activeTab === 'ARCHIVED' ? 'bg-rose-100 text-rose-900' : 'bg-slate-200 text-slate-700'
              }`}
            >
              {archivedCount}
            </span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-2.5 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2 min-w-0">
                      <img
                        src={l.seller?.avatarUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80'}
                        alt={l.seller?.name}
                ...
                {displayedListings.map((l: any) => {
                  ...
                  return (
                    <tr key={l.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="p-4 font-mono font-black text-brand-700">{formatIDR(l.price)}</td>
                      <td className="p-4">
        ...
        >
          ...
          >
            <div className="flex items-start justify-between border-b border-slate-200 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-slate-400">Nonaktifkan katalog dari pencarian</span>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Iklan Target:</span>
              <p className="font-bold text-xs text-slate-900 line-clamp-2">{takedownModalListing.title}</p>
              <span className="text-[11px] text-brand-700 font-mono font-black">{formatIDR(takedownModalListing.price)}</span>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              >
                <option value="Barang Tiruan / Indikasi KW Super">Barang Tiruan / Indikasi KW Super</option>
                <option value="Foto Iklan Tidak Jelas / Mengambil dari Google">Foto Iklan Tidak Jelas / Mengambil dari Google</option>
                <option value="Indikasi Penipuan / Kontak Luar Platform">Indikasi Penipuan / Mencantumkan Kontak Luar</option>
                <option value="Kategori Barang Terlarang / Ilegal">Kategori Barang Terlarang / Ilegal</option>
                <option value="Deskripsi Menyesatkan / Tidak Lengkap">Deskripsi Menyesatkan / Tidak Lengkap</option>
              </select>
            </div>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <button
                type="button"
                disabled={isProcessingAction}
                onClick={() => handleUpdateStatus(takedownModalListing.id, 'ARCHIVED', takedownModalListing.title)}
