          ...
          return (
            <li key={index} className="inline-flex items-center gap-1.5 min-w-0 shrink-0">
              ...
              {item.href && !isLast ? (
                <Link
                  href={item.href}
                  className="hover:text-brand-600 transition-colors whitespace-nowrap p-1 rounded-md hover:bg-slate-100/70"
                >
                  {item.label}
                </Link>
              ) : (
                <span
                  className="font-bold text-slate-900 whitespace-nowrap truncate max-w-[75vw] sm:max-w-md"
